                    ...
                    <>
                      <li>
                        ...
                        {props.openingHours.length > 0 && props.openingHours.map((item, i)=>(
                          <p style={{textTransform:"capitalize"}} key={i}>{localeMomentEventDates(item.date, props.event.language_id)}</p>
                          ))}
                      </li>
                      <li>
                        ...
                        {props.openingHours.length > 0 && props.openingHours.map((item, i)=>(
                          <p style={{textTransform:"capitalize"}} key={i}>{localeMomentOpeningHours(item.date, props.event.language_id)} {" "} {`${moment(item?.date + ' ' + item?.start_time).format('HH:mm')} - ${moment(item?.date + ' ' + item?.end_time).format('HH:mm')}`}</p>
                        ))}
                      </li>
                    </>
                  }
