        ...
        <React.Fragment>
          <div className="ebs-countdown-wrapp d-flex w-100 countdown-wrapp ebs-counter-v10">
            {Math.floor(days / 30) > 0 && <span className="edgtf-countdown is-countdown">
              <span className="countdown-amount rounded-circle border bg-transparent"><div className="position-absolute top-50 start-50 translate-middle lh-base text-white">{zeroPad(Math.floor(days / 30))}<span className="countdown-period m-0 text-white">{event?.labels?.COUNTDOWN_LABEL_MONTHS}</span></div></span>
            </span>}
            <span className="edgtf-countdown is-countdown">
              <span className="countdown-amount rounded-circle border bg-transparent"><div className="position-absolute top-50 start-50 translate-middle lh-base text-white">{zeroPad(Math.floor(days % 30))} <span className="countdown-period m-0 text-white">{event?.labels?.COUNTDOWN_LABEL_DAYS}</span></div></span>
              
            </span>
            <span className="edgtf-countdown is-countdown">
              <span className="countdown-amount rounded-circle border bg-transparent"><div className="position-absolute top-50 start-50 translate-middle lh-base text-white">{zeroPad(hours)}<span className="countdown-period m-0 text-white">{event?.labels?.COUNTDOWN_LABEL_HOURS}</span></div></span>
              
            </span>
            <span className="edgtf-countdown is-countdown">
              <span className="countdown-amount rounded-circle border bg-transparent"><div className="position-absolute top-50 start-50 translate-middle lh-base text-white">{zeroPad(minutes)}<span className="countdown-period m-0 text-white">{event?.labels?.COUNTDOWN_LABEL_MINUTES}</span></div></span>
              
            </span>
            <span className="edgtf-countdown is-countdown">
              <span className="countdown-amount rounded-circle border bg-transparent"><div className="position-absolute top-50 start-50 translate-middle lh-base text-white">{zeroPad(seconds)}<span className="countdown-period m-0 text-white">{event?.labels?.COUNTDOWN_LABEL_SECONDS}</span></div></span>
 ...
 const expiryDate = event.count_down_section && event.count_down_section.expiry_date
  ...
  const isValidDate = expiryDate && !isNaN(expiryDate.getTime());
    ...
    <div style={bgStyle} className="edgtf-parallax-section-holder ebs-bg-holder ebs-default-padding ebs-counter-holder ebs-master-default-wrapper">
      <div className="position-relative pt-4 w-100" style={{zIndex: 5}}>
