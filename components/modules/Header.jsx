  ...
  const top_menu =  useMemo(()=>{
    ...
    let menu = event.header_data.top_menu.map((item)=>{
      ...
      if(!['practicalinformation', 'additional_information', 'general_information', 'info_pages'].includes(item.alias)){
        ...
        console.log(page);
        rItem['menu_url'] =  page !== null && page['submenu'].length == 1 && page['submenu'][0].page_type !== "menu" ? (page['submenu'][0].page_type === 2 ? `/${event.url}/${item.alias}/${page['submenu'][0].id}` : `${page.website_protocol}${page['submenu'][0].url}`) : `/${event.url}/${item.alias}`;
