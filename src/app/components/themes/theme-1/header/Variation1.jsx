      ...
      if (panel.style.maxHeight) {
        ...
        for (var iii = 0; iii < coursePanel.length; iii++) {
          ...
          if (coursePanel[iii] === panelParent) {
            coursePanel[iii].style.maxHeight = coursePanel[iii].scrollHeight + panel.scrollHeight + 'px';
          }
        }
        ...
        <div className='container'>
          <div className='row d-flex align-items-center'>
            <div className='col-lg-3 col-6'>
              <div style={{ padding: '0', border: 'none' }} className='ebs-logo-main text-left'>
                <Link to={`/${event.url}`}>
              ...
              <nav className='navbar navbar-expand-lg navbar-light'>
                <button
                  ...
                  {!this.state.showMenu ? (
                    ...
                    <span style={{ fontSize: 34, marginTop: 2, lineHeight: 1, height: 30 }} className='material-icons'>
                      close
                    </span>
                  )}
                </button>
                        ...
                        {menu.alias === 'myaccount' && (
                          <ul className='dropdown-menu'>
                            {!this.props.userExist ? (
                              menus['my_account_sub_menu'].map((myaccount, k) => (
                                <li className='nav-item' key={k}>
                              ...
                              <li className='nav-item'>
                                <NavLink aria-current='page' className='nav-link' to={`/${event.url}/profile`}>
                                  My Profile
                                </NavLink>
                              </li>
                            )}
                            ...
                            {menus['practical_info_menu'].map((pItem, k) =>
                              pItem.page_type && pItem.page_type === 'menu' ? (
                                <li className='nav-item' key={pItem.id}>
                                  <span className='nav-link'>{pItem.info.name}</span>
                                        ...
                                        <li className='nav-item' key={k}>
                                          {subitem.page_type && subitem.page_type === 2 ? (
                                            ...
                                            <NavLink
                                              aria-current='page'
                                              className='nav-link'
                                              to={'/' + this.props.event.url + '/' + menu.alias + '/' + subitem.id}
                                              key={subitem.id}>
                                              {subitem.info.name}
                                            </NavLink>
                                          )}
                                ...
                                <li className='nav-item' key={k}>
                                  {pItem.page_type && pItem.page_type === 2 ? (
                                    <a className='nav-link' aria-current='page' href={`${pItem.website_protocol}${pItem.url}`}>
                                      {pItem.info.name}
                                    </a>
                                  ) : (
                                    <NavLink
                                      aria-current='page'
                                      className='nav-link'
                                      to={'/' + this.props.event.url + '/' + menu.alias + '/' + pItem.id}
                                      key={pItem.id}>
                                      {pItem.info.name}
                                    </NavLink>
                                  )}
                            ...
                            {menus['additional_info_menu'].map((aItem, k) =>
                              aItem.page_type && aItem.page_type === 'menu' ? (
                                <li className='nav-item' key={aItem.id}>
                                  <span className='nav-link'>{aItem.info.name}</span>
                                ...
                                <li className='nav-item' key={k}>
                                  {aItem.page_type && aItem.page_type === 2 ? (
                                    <a className='nav-link' aria-current='page' href={`${aItem.website_protocol}${aItem.url}`}>
                                      {aItem.info.name}
                                    </a>
                                  ) : (
                                    <NavLink
                                      aria-current='page'
                                      className='nav-link'
                                      to={'/' + this.props.event.url + '/' + menu.alias + '/' + aItem.id}
                                      key={aItem.id}>
                                      {aItem.info.name}
                                    </NavLink>
                                  )}
                            ...
                            {menus['general_info_menu'].map((gItem, k) =>
                              gItem.page_type && gItem.page_type === 'menu' ? (
                                <li className='nav-item' key={gItem.id}>
                                  <span className='nav-link'>{gItem.info.name}</span>
                                ...
                                <li className='nav-item' key={k}>
                                  {gItem.page_type && gItem.page_type === 2 ? (
                                    <a className='nav-link' aria-current='page' href={`${gItem.website_protocol}${gItem.url}`}>
                                      {gItem.info.name}
                                    </a>
                                  ) : (
                                    <NavLink
                                      aria-current='page'
                                      className='nav-link'
                                      to={'/' + this.props.event.url + '/' + menu.alias + '/' + gItem.id}
                                      key={gItem.id}>
                                      {gItem.info.name}
                                    </NavLink>
                                  )}
